...
int ip(const Block & block , HBlock & left , HBlock & right)
{
	for(size_t i = 0 ; i < right.size() ; ++i)
		right[i] = block[ip_table[i] - 1] ;//right
	for(size_t i = 0 ; i < left.size() ; ++i)
		left[i] = block[ip_table[i + left.size()] - 1] ;//left
	return 0 ;
}
...
{	
	...
	memset(key , 0 , 8);
		...
		ostrstream strout(origin,8);  
	
      	strout<<"123";
	    strout<<ends;
		 

		BlockFromStr(block , origin) ;
		des(block , bkey , e) ;
